...
const ConfirmSectionView: React.FC = () => {
  ...
  const [mailState, handleSubmit, reset] = useForm<Inputs>('mldpoewd');
  ...
  return (
    ...
    >
      ...
      <form onSubmit={handleSubmit} className="flex flex-col w-full px-4 space-y-3 pt-8">
        ...
        <select id="confirm" defaultValue={''} name="Xác nhận" className="border rounded-md px-2 py-1">
        ...
        <select id="included" defaultValue={''} name="Đi cùng với" className="border rounded-md px-2 py-1">
          <option disabled value="">
            Bạn tham dự cùng ai?
          </option>
        ...
        <Tooltip id="qr" clickable={true} openOnClick={true} style={{ backgroundColor: '#8b0909', padding: 0 }}>
          <div className="flex w-[380px] items-center space-x-2 p-2">
            ...
            <div className="flex flex-col w-1/2 justify-center items-center border p-2 rounded-xl space-y-1">
              <Image
                src="https://img.vietqr.io/image/970415-104602592001-qr_only.png"
                alt="bank2"
                width={0}
                height={0}
                sizes="100vw"
              ...
              <span className={`${fontEbgaramond.className}`}>VietinBank</span>
              <span className={`${fontEbgaramond.className}`}>104602592001</span>
            </div>
          </div>
        </Tooltip>
      </AnimationOnScroll>
